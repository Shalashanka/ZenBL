...
class ZentoxOverlayService : Service() {
    private var windowManager: WindowManager? = null
    private var overlayView: View? = null
    private val mainHandler = Handler(Looper.getMainLooper())
    private val emergencyHoldRunnable = Runnable {
        ZentoxManager.requestEmergencyBreak()
        hideOverlay()
    }
    ...
    companion object {
        private const val TAG = "ZentoxOverlayService"
        private const val EMERGENCY_HOLD_MS = 5_000L
        private const val ACTION_SHOW = "com.zentox.engine.action.OVERLAY_SHOW"
        private const val ACTION_HIDE = "com.zentox.engine.action.OVERLAY_HIDE"
        private const val EXTRA_PACKAGE_NAME = "packageName"
        @Volatile
        private var overlayVisible: Boolean = false

        fun show(context: Context, packageName: String) {
            val intent = Intent(context, ZentoxOverlayService::class.java).apply {
                action = ACTION_SHOW
